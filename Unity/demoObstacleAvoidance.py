# -*- encoding: utf-8 -*-
"""
receive the simulated data from Unity and drive (send motion commands) the avatar to move
"""
import threading
import time
    ...
    while True:
        try:
            if not qq1.empty():
                ...
                if Vis:
                    _tmp = _depth_image_matrix / depth_scale
                    depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(_tmp, alpha=0.03), cv2.COLORMAP_JET)
                    tools.vis_Unity_OA(direction_togo, 0.12, depth_colormap, _depth_image_matrix, walkable_7=walkable_7)
                if direction_togo == -1:
                    sen_msg("backward")
                if direction_togo == 0:
                    tools.play_by_direction(-25, _listener, source1100, sleep_time=tools.audio_sleep_time)
                    sen_msg("left")
                if direction_togo == 1:
                    tools.play_by_direction(-25, _listener, source1100, sleep_time=tools.audio_sleep_time)
                    sen_msg("left")
...
def detect():
    ...
    depth_frame_ndarray = upsample(depth_frame_str, 1920, 1080, 5, 5, 2.8)
