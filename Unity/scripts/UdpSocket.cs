...
public class UdpSocket : MonoBehaviour
{
    ...
    private void ProcessInput(string input)
    {
        ...
        if (input.Trim().StartsWith("motion_"))
        {
            ...
            motion_time = 0.7f;        //动画的执行时间 也是人行走的时间，此参数是有效的
            ...
            {
                case "forwardL":
                    vertical_ = 8f;//人行走的速度，但没有用，最终的速度由unity中手动指定
                    horizontal_ = 0f;
            ...
            characterController.ForwardInput = vertical_;  //给simpleCharacterController发送信号
            ...
            motion_type_time_count = (int)Convert.ToInt16(motion_time / 0.02f)-1;  // fixUpdate 0.02秒，算出需要多少个 这样的 update，用来计算运动时间以便及时终止
        ...
        if (input.Trim().StartsWith("saved_"))
        {
            ...
            characterController.TurnInput = horizontal_;
            characterController.JumpInput = jump_;
            // fixed update 似乎没有做到一秒钟刷很多次，所以我们这里手动进行位置刷新
